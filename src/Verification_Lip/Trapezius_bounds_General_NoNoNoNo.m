function [Lb, Ub, alpha_params, beta_params]= Trapezius_bounds_General(Center, epsilon, net, model_dim, cntrl_dim, border,  method_Trajectory, method_STL)
...
switch method_Trajectory
    ...
    case 'Linear-CROWN'
        unit_length = length(model_dim)-2+length(cntrl_dim)-2;
        dim=cntrl_dim(1);
        Nett = Net_connected(Net, border, model_dim, cntrl_dim);
        l0 = Center-epsilon;
        u0 = Center+epsilon;
        W_pos = 0.5*(Nett.weights{1}+abs(Nett.weights{1}));
        W_neg = 0.5*(Nett.weights{1}-abs(Nett.weights{1}));
        Lb{1} = W_pos*l0+W_neg*u0+Nett.biases{1};
        Ub{1} = W_pos*u0+W_neg*l0+Nett.biases{1};
        
        S_Lb{1}=l0;
        S_Ub{1}=u0;
        
        [param_l_linear{1}, param_u_linear{1} ] = linear_lines( Lb{1}, Ub{1}, Nett.layers{1} );
        [alpha_params{1},beta_params{1}] =   piecewise_linear_slopes(Lb{1}, Ub{1}, Nett.layers{1});
        ind=1;
        for i = 1:border-2
            disp(i)
            [Lb{i+1}, Ub{i+1}] = Linear_CROWN(Nett, i+1 , l0, u0, param_l_linear, param_u_linear);
            [param_l_linear{i+1}, param_u_linear{i+1} ] = linear_lines( Lb{i+1}, Ub{i+1}, Nett.layers{i+1} );
            [alpha_params{i+1},beta_params{i+1}] =   piecewise_linear_slopes(Lb{i+1}, Ub{i+1}, Nett.layers{i+1});
            if mod(i+1, unit_length)==1
                ind=ind+1;
                S_Lb{ind}=Lb{i+1}(1:dim);
                S_Ub{ind}=Ub{i+1}(1:dim);
            end 
...
%         the_Ub=[];
%         for i=1:ind  
%             the_Lb=[the_Lb;S_Lb{i}];
%             the_Ub=[the_Ub;S_Ub{i}];
%         end
%         the_Lb = [the_Lb; Lb{end}(dim+1:end)];
%         the_Ub = [the_Ub; Ub{end}(dim+1:end)];
        post_Lb = L_str2func(Net.layers{border-1}, Lb{end});
        post_Ub = L_str2func(Net.layers{border-1}, Ub{end});
        W_pos = 0.5*(Net.weights{border}+abs(Net.weights{border}));
        W_neg = 0.5*(Net.weights{border}-abs(Net.weights{border}));
        Lb{border} = W_pos*post_Lb+W_neg*post_Ub+Net.biases{border};
        Ub{border} = W_pos*post_Ub+W_neg*post_Lb+Net.biases{border};
        [alpha_params{border} ,beta_params{border}]  =    piecewise_linear_slopes(Lb{border}, Ub{border}, Net.layers{border});
...
for jj = 1:the_end-start+1
    ...
    for k = 1:lenS
        ...
        [Out_layer, ~] = F.reach(In_layer, analysis_type,6);
...
for j = 1:len1
    ...
    cd C:\Users\Navid\Documents\MATLAB\Others\Personal_Trapezius_toolbox_time_variant
end
...
function  Net = Net_connected(net, border, model_dim, cntrl_dim)

unit_dim = [cntrl_dim(2:end-1) model_dim(2:end-1)];
unit_len = length(unit_dim);
dim=cntrl_dim(1);
Net.weights{1}=net.weights{1};
Net.biases{1}=net.biases{1};
Net.layers{1}=net.layers{1};

for i = 2:border-1
    ii=mod(i, unit_len);
    if ii==0
        first = unit_dim(unit_len-1)+dim;
        second = unit_dim(unit_len)+dim;
    elseif ii==1
        first = unit_dim(unit_len)+dim;
        second = unit_dim(ii)+dim;
    else
        first = unit_dim(ii-1)+dim;
        second = unit_dim(ii)+dim;
    end
    Net.weights{i}=net.weights{i}(end-(second-1):end  , end-(first-1):end );
    Net.biases{i}=net.biases{i}(end-(second-1):end  , 1 );
    Net.layers{i}=net.layers{i}(end-(second-1):end );
end
end
