function [decision, Letsgo, counter_example]  =  Lip_Verify_lip_fixed( net, border, lb, ub, M, method_Trajectory, method_STL, LipSDP_type, certificate1 )
...
while ~ready
    ...
    certificate =  eval(['Rho2(' array2index(v) ')/certificate1']);
    
    if eps>certificate
        [decision, Letsgo, counter_example]  =  Lip_Verify_lip_fixed(net,border, eval(['Center{' array2index(v) '}-epsilon{' array2index(v) '}']), eval(['Center{' array2index(v) '}+epsilon{' array2index(v) '}' ] ), M, method_Trajectory, method_STL, LipSDP_type, certificate1 );
    end
%     save2 = eval(['Rho2(' array2index(v) ')']);
%     save3 = eval(['Center{' array2index(v) '}']);
%     save4 = eval(['epsilon{' array2index(v) '}']);
%     Filename = naming(save3,save4);
%     save( Filename   ,  'save2' , 'save3' , 'save4')
